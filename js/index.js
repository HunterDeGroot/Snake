...
var go = function(images) {
	...
	function paint() {
		...
		ctx.drawImage(logo, (1 / 2) * w - (338/2), (1 / 2) * h - 60);
		...
		if (nx == -1 || nx >= w / cw || ny == -1 || ny >= h / cw || check_collision(nx, ny, snake_array)) {
			...
			if (tries == 4) {
				tries = 0;
				alert("Good try! You zapped " + score + " bacteria cells. Press ok to go another four rounds.");
				ctx.fillStyle = "white";
				ctx.fillRect(0, 0, w, h);
		...
		for (var i = 0; i < snake_array.length; i++) {
			...
			paint_cell(c.x, c.y, "n");
		}

		paint_cell(food.x, food.y, "y");

		//Lets paint the score
		var score_text = "Score: " + score;
		ctx.font = '20pt futura'
		ctx.fillText(score_text, 5, h - 5);
	}

	//Lets first create a generic function to paint cells
	function paint_cell(x, y, q) {
		if (q == "y") {

			ctx.drawImage(img, x * cw - 50, y * cw - 50);
